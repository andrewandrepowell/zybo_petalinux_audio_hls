/*
 * This application runs a small audio demo that utilizes
 * the ADI I2S core, SSM2603 audio codec, and HLS FIR core.
 *
 * Unfortunately, there is a problem with the application in that
 * , when ran the first time after boot, the sound is distorted. Once
 * the application is closed and then restarted, there are no issues.
 * The application will continue to operate correctly until the system
 * is entirely restarted ( e.g. shutting down and booting the kernel again ).
 */
/* C++ Includes. */
#include <iostream>
#include <stdexcept>
...
vector<int32_t> rec_samples( SAMPLES_TOTAL );
